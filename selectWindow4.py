...
def select(time: list, data: list, bandwidth: float, limit: float, mode: str):
    ...
    for i in range(length):
        ...
        best_quality_out_full.append(out_full[res[0][0]][i])
        res = sorted(compare, reverse=True, key=lambda x: x[0])
        max_window_out.append((int(res[0][0]), res[0][1]))
        max_window_out_full.append(out_full[res[0][0]][i])
        min_window_out.append((int(res[-1][0]), res[-1][1]))
        min_window_out_full.append(out_full[res[-1][0]][i])

    return best_quality_out_full, max_window_out_full, min_window_out_full, [(int(n_frame[0]), n_frame[1]) for n_frame in n_frames], total
